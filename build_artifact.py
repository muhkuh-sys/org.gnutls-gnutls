...
for strMatch in glob.iglob(os.path.join(strCwd, 'gnutls-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'gnutls-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "gnutls-*.xml".')
